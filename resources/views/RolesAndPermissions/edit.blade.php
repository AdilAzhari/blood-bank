<x-app-layout>
    <x-form.breadcrumb :items="['Home', 'Roles', 'Edit']" :routes="['/', '/create-roles', route('create-roles')]" />
    <x-alert />
        ...
        <div class="bg-white dark:bg-gray-800 rounded-lg shadow-md p-6">
            ...
            <form method="POST" action="{{ URL('update-role') }}">
                ...
                <div class="mb-4">
                    ...
                    <input type="text" required name="name" class="mt-1 block w-full px-3 py-2 border border-gray-300 rounded-md shadow-sm focus:outline-none focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white" value="{{ $role->name }}" />
                </div>
                <div class="mb-4">
                    <label for="description" class="block text-sm font-medium text-gray-700 dark:text-gray-300">{{ __('Role Description') }}</label>
                    <textarea required name="description" class="mt-1 block w-full px-3 py-2 border border-gray-300 rounded-md shadow-sm focus:outline-none focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white">{{ $role->description }}</textarea>
                </div>
                                ...
                                <tbody>
                                    @foreach ($permissions as $permission)
                                        <tr class="border-b dark:border-gray-700 odd:bg-white odd:dark:bg-gray-900 even:bg-gray-50 even:dark:bg-gray-800">
                                            ...
                                            <td class="px-4 py-3">
                                                <input type="checkbox" value="{{ $permission->name }}" name="permission[]" id="permission_{{ $permission->id }}" @if ($role->permissions->contains('id', $permission->id)) checked @endif class="rounded border-gray-300 text-indigo-600 shadow-sm focus:border-indigo-300 focus:ring focus:ring-indigo-200 focus:ring-opacity-50 dark:bg-gray-700 dark:border-gray-600 dark:text-gray-300 dark:focus:ring-indigo-600" />
                                            </td>
                                        </tr>
                                    @endforeach
                                </tbody>
